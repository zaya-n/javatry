...
public class Step07ExceptionTest extends PlainTestCase {
     ...
     *
     * ↑　And What is exception class name displayed at the last "Caused By:" of stack trace?　→ And What exception class name is displayed at the last "Caused By:" of stack trace?
     */
     ...
     *
     * ↑ Execute this test and read exception message and write situation and cause on comment.　→　Execute the test and write situation and cause of the exception in comment by reading the exception message.　
     */
    public void test_exception_translation_debugChallenge() {
        try {
            ...
            // - - - - - - - - - -
            // screwSpecが"\(^_^)/"になっているため、screwを作れず、そのためSteering Wheelも作れず→　そのため車を作ることができず　→　車を購入することができない
            //
            // SupercarClient().buySupercar()
            // → SupercarDealerでdealer.orderSuperCar("steering wheel is like sea")
            // →　SupercarManufacturerでmanufacturer.makeSupercar("piari")、SupercarEasyCatalogでsteeringWheelSpecId=3
            // → SupercarSteeringWheelManufacturerでmakeSteeringWheel(3)、SupercarSteeringWheelComponentDBでclincherSpecText = "\(^_^)/"、
            // → SpecialScrewManufacturerでmakeSpecialScrew("\(^_^)/"
            // ---> throw new SpecialScrewCannotMakeBySpecException("The kawaii face is not useful to make screw: ScrewSpec:{\(^_^)/}");
            //
            // _/_/_/_/_/_/_/_/_/_/
        }
    }
